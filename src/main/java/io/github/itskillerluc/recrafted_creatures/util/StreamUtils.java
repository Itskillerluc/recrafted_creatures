...
public class StreamUtils {
     ...
     *
     * @param classCtx The classCtx to run from. Uses the given instances class loader as the class loader for the pool.
     */
    ...
    public static <T> T executeUntil(final Callable<T> callable, long maxTime, TimeUnit unit, T fallback, Supplier<String> timeoutMsg) {
        ...
        try {
            ...
            LOGGER.error(timeoutMsg.get());
            ex.printStackTrace();
            dumpFSThreads();
            return fallback;
        }
        catch (ExecutionException e) {
            LOGGER.error("Exception during multithreaded recipe lookup");
            e.printStackTrace();
    ...
    public static void dumpFSThreads() {
        ...
        LOGGER.debug(Arrays.stream(infos).filter(info -> info.getThreadName().startsWith("FastSuite")).map(Object::toString).collect(Collectors.joining()));
    }
